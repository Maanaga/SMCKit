...
extension Double {
    ...
    /// SMC `sp78` is typically interpreted as a signed 16-bit fixed-point value (scale 256).
    init(fromSP78 bytes: SP78) {
        let combined = UInt16(bytes.0) << 8 | UInt16(bytes.1)
        self = Double(Int16(bitPattern: combined)) / 256.0
    }
}
...
public struct SMCKit {
    ...
    /// Connection to the SMC driver (legacy global; not thread-safe — serialize calls from one task).
...
public struct TemperatureSensors {
    ...
    public static let all: [FourCharCode: TemperatureSensor] = [
...
extension SMCKit {
    ...
    public static func fanName(_ id: Int) throws -> String {
        ...
        let c1 = String(UnicodeScalar(UInt32(data.4))!)
        let c2 = String(UnicodeScalar(UInt32(data.5))!)
        let c3 = String(UnicodeScalar(UInt32(data.6))!)
        let c4 = String(UnicodeScalar(UInt32(data.7))!)
        let c5 = String(UnicodeScalar(UInt32(data.8))!)
        let c6 = String(UnicodeScalar(UInt32(data.9))!)
        let c7 = String(UnicodeScalar(UInt32(data.10))!)
        let c8 = String(UnicodeScalar(UInt32(data.11))!)
        let c9 = String(UnicodeScalar(UInt32(data.12))!)
        let c10 = String(UnicodeScalar(UInt32(data.13))!)
        let c11 = String(UnicodeScalar(UInt32(data.14))!)
        let c12 = String(UnicodeScalar(UInt32(data.15))!)
...
extension SMCKit {
    ...
    public static func batteryInformation() throws -> BatteryInfo {
        ...
        let isCharging = batteryInfoData.0 & 1 == 1
        let isACPresent = (batteryInfoData.0 >> 1) & 1 == 1
        let isBatteryOk = (batteryInfoData.0 >> 6) & 1 == 1
